...
// Fallback search replace for the blob
const blobOld = '<Blob className={bsolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 -rotate-12 opacity-80 group-hover:scale-125 group-hover:rotate-6 transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] }/>';
const blobNew = '<Blob className={bsolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[130%] h-[130%] animate-[spin_40s_linear_infinite] opacity-90 group-hover:scale-[1.1] transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] }/>';
file = file.split(blobOld).join(blobNew);

const iconOld = '<div className={w-20 h-20 bg-white dark:bg-slate-800 rounded-3xl shadow-sm flex items-center justify-center relative z-10 group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-500 border-2 border-white dark:border-slate-800 ring-4 ring-white/50 dark:ring-slate-800/50 }>\n                <Icon className="w-10 h-10" />\n              </div>';
const iconNew = <div className={\w-20 h-20 bg-white dark:bg-slate-800/90 backdrop-blur-sm rounded-3xl shadow-[6px_6px_0px_rgba(0,0,0,0.06)] dark:shadow-[6px_6px_0px_rgba(0,0,0,0.3)] flex items-center justify-center relative z-10 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 border-2 border-slate-100 dark:border-slate-700 \\}>\n                <Icon className="w-10 h-10 drop-shadow-sm" />\n              </div>;
